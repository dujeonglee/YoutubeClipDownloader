...
private:
    ...
    void _perform_for_all_key_data(avltreeelement<KEY, DATA>* node, std::function <void (KEY&, DATA&)> func){
        func(node->_key, node->_data);
        if(node->_left){
            _perform_for_all_key_data(node->_left, func);
        }
        if(node->_right){
            _perform_for_all_key_data(node->_right, func);
        }
    }
...
public:
    ...
    void perform_for_all_key_data(std::function <void (KEY&, DATA&)> func){
        if(_root){
            _perform_for_all_key_data(_root, func);
        }
    }
